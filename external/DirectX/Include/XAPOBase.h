...
#define XAPOBASE_DEFAULT_FLAG                                                                                          \
    (XAPO_FLAG_CHANNELS_MUST_MATCH | XAPO_FLAG_FRAMERATE_MUST_MATCH | XAPO_FLAG_BITSPERSAMPLE_MUST_MATCH |             \
...
private:
    ...
    BOOL
        m_fIsScalarMatrix; // TRUE if m_pfl32MatrixCoefficients is diagonal matrix with all main diagonal entries equal, i.e. m_pfnMatrixMixFunction only used for type conversion (no channel conversion), used for thru processing
...
private:
    BYTE *
        m_pParameterBlocks; // three contiguous process parameter blocks used for synchronization, user responsible for initialization of parameter blocks before IXAPO::Process/SetParameters/GetParameters called
    BYTE *m_pCurrentParameters; // pointer to current process parameters, must be aligned for atomic operations
    BYTE *
        m_pCurrentParametersInternal; // pointer to current process parameters (temp pointer read by SetParameters/BeginProcess/EndProcess)
    ...
    BOOL
        m_fNewerResultsReady; // TRUE if there exists new processing results not yet picked up by GetParameters(), must be aligned for atomic operations
...
public:
    ...
    ////
    CXAPOParametersBase(const XAPO_REGISTRATION_PROPERTIES *pRegistrationProperties,
