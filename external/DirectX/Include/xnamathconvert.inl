...
#define XM_PACK_UNSIGNEDN_SCALE(BitsX, BitsY, BitsZ, BitsW)                                                            \
    {                                                                                                                  \
        -(FLOAT)((1 << (BitsX)) - 1) / XM_PACK_FACTOR, -(FLOAT)((1 << (BitsY)) - 1) / XM_PACK_FACTOR,                  \
            -(FLOAT)((1 << (BitsZ)) - 1) / XM_PACK_FACTOR, -(FLOAT)((1 << (BitsW)) - 1) / XM_PACK_FACTOR               \
    }

#define XM_PACK_SIGNEDN_SCALE(BitsX, BitsY, BitsZ, BitsW)                                                              \
    {                                                                                                                  \
        -(FLOAT)((1 << ((BitsX)-1)) - 1) / XM_PACK_FACTOR, -(FLOAT)((1 << ((BitsY)-1)) - 1) / XM_PACK_FACTOR,          \
            -(FLOAT)((1 << ((BitsZ)-1)) - 1) / XM_PACK_FACTOR, -(FLOAT)((1 << ((BitsW)-1)) - 1) / XM_PACK_FACTOR       \
    }
...
#pragma warning(push)
#pragma warning(                                                                                                       \
        disable : 4701) // Prevent warnings about 'Result' potentially being used without having been initialized
...
#else // _XM_SSE_INTRINSICS_
    XMASSERT(MulExponent < 32);
    static const XMVECTORF32 MaxInt = {65536.0f * 32768.0f - 128.0f,
...
#else // _XM_SSE_INTRINSICS_
    XMASSERT(DivExponent < 32);
    static const XMVECTORF32 FixUnsigned = {32768.0f * 65536.0f,
                                            32768.0f * 65536.0f,
                                            32768.0f * 65536.0f,
                                            32768.0f * 65536.0f};
...
#else // _XM_SSE_INTRINSICS_
    XMASSERT(MulExponent < 32);
    static const XMVECTORF32 MaxUInt = {65536.0f * 65536.0f - 256.0f,
                                        65536.0f * 65536.0f - 256.0f,
                                        65536.0f * 65536.0f - 256.0f,
                                        65536.0f * 65536.0f - 256.0f};
    static const XMVECTORF32 UnsignedFix = {32768.0f * 65536.0f,
                                            32768.0f * 65536.0f,
                                            32768.0f * 65536.0f,
                                            32768.0f * 65536.0f};
...
XMFINLINE XMVECTOR XMLoadHalf2(CONST XMHALF2 *pSource)
{
    ...
    XMASSERT(pSource);
    {
        XMVECTOR vResult = {XMConvertHalfToFloat(pSource->x), XMConvertHalfToFloat(pSource->y), 0.0f, 0.0f};
        return vResult;
    }
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pSource);
...
XMFINLINE XMVECTOR XMLoadShortN2(CONST XMSHORTN2 *pSource)
{
    ...
    {
        XMVECTOR vResult = {(FLOAT)pSource->x * (1.0f / 32767.0f), (FLOAT)pSource->y * (1.0f / 32767.0f), 0.0f, 0.0f};
        return vResult;
    }
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 UHenDN3Mul = {1.0f / 2047.0f,
                                           1.0f / (2047.0f * 2048.0f),
                                           1.0f / (1023.0f * 2048.0f * 2048.0f),
                                           0};
    XMASSERT(pSource);
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 HenDN3Mul = {1.0f / 1023.0f,
                                          1.0f / (1023.0f * 2048.0f),
                                          1.0f / (511.0f * 2048.0f * 2048.0f),
                                          0};
    XMASSERT(pSource);
    XMASSERT((pSource->v & 0x7FF) != 0x400);
    XMASSERT(((pSource->v >> 11) & 0x7FF) != 0x400);
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 UDHenN3Mul = {1.0f / 1023.0f,
                                           1.0f / (2047.0f * 1024.0f),
                                           1.0f / (2047.0f * 1024.0f * 2048.0f),
                                           0};
    XMASSERT(pSource);
    // Get the 32 bit value and splat it
    XMVECTOR vResult = _mm_load_ps1(reinterpret_cast<const float *>(&pSource->v));
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 DHenN3Mul = {1.0f / 511.0f,
                                          1.0f / (1023.0f * 1024.0f),
                                          1.0f / (1023.0f * 1024.0f * 2048.0f),
                                          0};
    XMASSERT(pSource);
    XMASSERT((pSource->v & 0x3FF) != 0x200);
    XMASSERT(((pSource->v >> 10) & 0x7FF) != 0x400);
...
XMFINLINE XMVECTOR XMLoadHalf4(CONST XMHALF4 *pSource)
{
    ...
    XMASSERT(pSource);
    {
        XMVECTOR vResult = {XMConvertHalfToFloat(pSource->x),
...
XMFINLINE XMVECTOR XMLoadShortN4(CONST XMSHORTN4 *pSource)
{
    ...
    {
        XMVECTOR vResult = {(FLOAT)pSource->x * (1.0f / 32767.0f),
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pSource);
    static const XMVECTORF32 FixupY16W16 = {1.0f / 65535.0f,
                                            1.0f / 65535.0f,
...
XMFINLINE XMVECTOR XMLoadXIcoN4(CONST XMXICON4 *pSource)
{
    ...
    XMASSERT(((pSource->v >> 20) & 0xFFFFFull) != 0x80000ull);
    XMASSERT(((pSource->v >> 40) & 0xFFFFFull) != 0x80000ull);
    static const XMVECTORF32 LoadXIcoN4Mul = {1.0f / 524287.0f,
                                              1.0f / (524287.0f * 4096.0f),
                                              1.0f / 524287.0f,
                                              1.0f / (15.0f * 4096.0f * 65536.0f)};
    XMASSERT(pSource);
    ...
    XMVECTOR vTemp = _mm_shuffle_ps(reinterpret_cast<const __m128 *>(&vResultd)[0],
                                    reinterpret_cast<const __m128 *>(&vResulti)[0],
                                    _MM_SHUFFLE(1, 0, 1, 0));
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 LoadUIcoN4Mul = {1.0f / 1048575.0f,
                                              1.0f / (1048575.0f * 4096.0f),
                                              1.0f / 1048575.0f,
                                              1.0f / (15.0f * 4096.0f * 65536.0f)};
    XMASSERT(pSource);
    // Grab the 64 bit structure
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 LoadIcoN4Mul = {1.0f / 524287.0f,
                                             1.0f / (524287.0f * 4096.0f),
                                             1.0f / 524287.0f,
                                             1.0f / (7.0f * 4096.0f * 65536.0f)};
    XMASSERT(pSource);
    // Grab the 64 bit structure
    __m128d vResultd = _mm_load_sd(reinterpret_cast<const double *>(&pSource->v));
    // By shifting down 8 bits, y and z are in seperate 32 bit elements
...
XMFINLINE XMVECTOR XMLoadDecN4(CONST XMDECN4 *pSource)
{
    ...
    XMASSERT(((pSource->v >> 20) & 0x3FF) != 0x200);
    XMASSERT(((pSource->v >> 30) & 0x3) != 0x2);
    static const XMVECTORF32 DecN4Mul = {1.0f / 511.0f,
                                         1.0f / (511.0f * 1024.0f),
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 LoadUByteN4Mul = {1.0f / 255.0f,
                                               1.0f / (255.0f * 256.0f),
                                               1.0f / (255.0f * 65536.0f),
...
#elif defined(_XM_SSE_INTRINSICS_)
    static const XMVECTORF32 LoadByteN4Mul = {1.0f / 127.0f,
                                              1.0f / (127.0f * 256.0f),
                                              1.0f / (127.0f * 65536.0f),
...
XMFINLINE XMVECTOR XMLoadColor(CONST XMCOLOR *pSource)
{
    ...
    XMASSERT(pSource);
    {
        ...
        XMVECTOR vColor = {(FLOAT)((iColor >> 16) & 0xFF) * (1.0f / 255.0f),
...
XMFINLINE XMMATRIX XMLoadFloat4x3(CONST XMFLOAT4X3 *pSource)
{
    ...
    XMMATRIX M(vTemp1, vTemp2, vTemp3, reinterpret_cast<const __m128 *>(&vTemp4i)[0]);
    return M;
#elif defined(XM_NO_MISALIGNED_VECTOR_ACCESS)
#endif // _XM_VMX128_INTRINSICS_
}
...
XMFINLINE VOID XMStoreUHenDN3(XMUHENDN3 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((UINT)N.vector4_f32[2] & 0x3FF) << 22) | (((UINT)N.vector4_f32[1] & 0x7FF) << 11) |
...
XMFINLINE VOID XMStoreHenDN3(XMHENDN3 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((INT)N.vector4_f32[2] & 0x3FF) << 22) | (((INT)N.vector4_f32[1] & 0x7FF) << 11) |
    ...
    vResult = _mm_shuffle_ps(reinterpret_cast<const __m128 *>(&vResulti)[0],
                             reinterpret_cast<const __m128 *>(&vResulti)[0],
                             _MM_SHUFFLE(0, 3, 2, 1));
...
XMFINLINE VOID XMStoreUDHenN3(XMUDHENN3 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((UINT)N.vector4_f32[2] & 0x7FF) << 21) | (((UINT)N.vector4_f32[1] & 0x7FF) << 10) |
...
XMFINLINE VOID XMStoreDHenN3(XMDHENN3 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((INT)N.vector4_f32[2] & 0x7FF) << 21) | (((INT)N.vector4_f32[1] & 0x7FF) << 10) |
...
XMFINLINE VOID XMStoreU565(XMU565 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((USHORT)N.vector4_f32[2] & 0x1F) << 11) | (((USHORT)N.vector4_f32[1] & 0x3F) << 5) |
...
XMFINLINE VOID XMStoreXIcoN4(XMXICON4 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = ((UINT64)N.vector4_f32[3] << 60) | (((INT64)N.vector4_f32[2] & 0xFFFFF) << 40) |
                      (((INT64)N.vector4_f32[1] & 0xFFFFF) << 20) | (((INT64)N.vector4_f32[0] & 0xFFFFF));

#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pDestination);
    // Note: Masks are x,w,y and z
    static const XMVECTORF32 MinXIcoN4 = {-1.0f, 0.0f, -1.0f, -1.0f};
    static const XMVECTORF32 ScaleXIcoN4 = {524287.0f,
                                            15.0f * 4096.0f * 65536.0f * 0.5f,
                                            524287.0f * 4096.0f,
                                            524287.0f};
...
#define XM_Offset (3.0f)
    ...
    pDestination->v = ((UINT64)N.vector4_f32[3] << 60) | (((UINT64)N.vector4_f32[2] & 0xFFFFF) << 40) |
                      (((UINT64)N.vector4_f32[1] & 0xFFFFF) << 20) | (((UINT64)N.vector4_f32[0] & 0xFFFFF));
...
#define XM_Offset (3.0f)
    ...
    pDestination->v = ((INT64)N.vector4_f32[3] << 60) | (((INT64)N.vector4_f32[2] & 0xFFFFF) << 40) |
                      (((INT64)N.vector4_f32[1] & 0xFFFFF) << 20) | (((INT64)N.vector4_f32[0] & 0xFFFFF));
...
XMFINLINE VOID XMStoreXDecN4(XMXDECN4 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = ((UINT)N.vector4_f32[3] << 30) | (((INT)N.vector4_f32[2] & 0x3FF) << 20) |
                      (((INT)N.vector4_f32[1] & 0x3FF) << 10) | (((INT)N.vector4_f32[0] & 0x3FF));
...
XMFINLINE VOID XMStoreUDecN4(XMUDECN4 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = ((UINT)N.vector4_f32[3] << 30) | (((UINT)N.vector4_f32[2] & 0x3FF) << 20) |
                      (((UINT)N.vector4_f32[1] & 0x3FF) << 10) | (((UINT)N.vector4_f32[0] & 0x3FF));

#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pDestination);
    static const XMVECTORF32 ScaleUDecN4 = {1023.0f,
...
XMFINLINE VOID XMStoreDecN4(XMDECN4 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = ((INT)N.vector4_f32[3] << 30) | (((INT)N.vector4_f32[2] & 0x3FF) << 20) |
                      (((INT)N.vector4_f32[1] & 0x3FF) << 10) | (((INT)N.vector4_f32[0] & 0x3FF));

#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pDestination);
    static const XMVECTORF32 ScaleDecN4 = {511.0f,
                                           511.0f * 1024.0f,
                                           511.0f * 1024.0f * 1024.0f,
                                           1.0f * 1024.0f * 1024.0f * 1024.0f};
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pDestination);
    static const XMVECTORF32 ScaleUByteN4 = {255.0f,
                                             255.0f * 256.0f * 0.5f,
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT(pDestination);
    static const XMVECTORF32 ScaleByteN4 = {127.0f,
                                            127.0f * 256.0f,
                                            127.0f * 256.0f * 256.0f,
...
XMFINLINE VOID XMStoreUNibble4(XMUNIBBLE4 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = (((USHORT)N.vector4_f32[3] & 0xF) << 12) | (((USHORT)N.vector4_f32[2] & 0xF) << 8) |
                      (((USHORT)N.vector4_f32[1] & 0xF) << 4) | (((USHORT)N.vector4_f32[0] & 0xF));
#endif !_XM_SSE_INTRINSICS_
}
...
XMFINLINE VOID XMStoreU555(XMU555 *pDestination, FXMVECTOR V)
{
    ...
    pDestination->v = ((N.vector4_f32[3] > 0.f) ? 0x8000 : 0) | (((USHORT)N.vector4_f32[2] & 0x1F) << 10) |
                      (((USHORT)N.vector4_f32[1] & 0x1F) << 5) | (((USHORT)N.vector4_f32[0] & 0x1F));
#endif !_XM_SSE_INTRINSICS_
}
...
XMFINLINE VOID XMStoreColor(XMCOLOR *pDestination, FXMVECTOR V)
{
    ...
    pDestination->c = ((UINT)N.vector4_f32[3] << 24) | ((UINT)N.vector4_f32[0] << 16) | ((UINT)N.vector4_f32[1] << 8) |
