...
#define XAPO_E_FORMAT_UNSUPPORTED                                                                                      \
    MAKE_HRESULT(SEVERITY_ERROR, FACILITY_XAPO, 0x01) // requested audio format unsupported
...
typedef struct XAPO_REGISTRATION_PROPERTIES
{
    ...
    UINT32
        MaxInputBufferCount; // maximum number of input buffers supported for processing, must be >= MinInputBufferCount
    UINT32
        MinOutputBufferCount; // minimum number of output buffers required for processing, can be 0, must match MinInputBufferCount when XAPO_FLAG_BUFFERCOUNT_MUST_MATCH used
    UINT32
        MaxOutputBufferCount; // maximum number of output buffers supported for processing, must be >= MinOutputBufferCount, must match MaxInputBufferCount when XAPO_FLAG_BUFFERCOUNT_MUST_MATCH used
...
typedef struct XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS
{
    ...
    UINT32
        MaxFrameCount; // maximum number of frames in respective buffer that IXAPO::Process would have to handle, irrespective of dynamic variable settings, can be 0
...
typedef struct XAPO_PROCESS_BUFFER_PARAMETERS
{
    ...
    UINT32
        ValidFrameCount; // number of frames of valid data, must be within respective [0, XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount], always XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount for CBR/user-defined XAPOs, does not affect BufferFlags
...
DECLARE_INTERFACE_(IXAPO, IUnknown)
{
    ...
    (THIS_ __deref_out XAPO_REGISTRATION_PROPERTIES * *ppRegistrationProperties) PURE;
    ...
    (THIS_ UINT32 InputLockedParameterCount,
     __in_ecount_opt(InputLockedParameterCount) const XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS *pInputLockedParameters,
     UINT32 OutputLockedParameterCount,
     __in_ecount_opt(OutputLockedParameterCount) const XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS *pOutputLockedParameters)
        PURE;
    ...
    (THIS_ UINT32 InputProcessParameterCount,
     __in_ecount_opt(InputProcessParameterCount) const XAPO_PROCESS_BUFFER_PARAMETERS *pInputProcessParameters,
     UINT32 OutputProcessParameterCount,
     __inout_ecount_opt(OutputProcessParameterCount) XAPO_PROCESS_BUFFER_PARAMETERS *pOutputProcessParameters,
     BOOL IsEnabled) PURE;
...
DECLARE_INTERFACE_(IXAPOParameters, IUnknown)
{
    ...
    (THIS_ __in_bcount(ParameterByteSize) const void *pParameters, UINT32 ParameterByteSize) PURE;
    ...
    (THIS_ __out_bcount(ParameterByteSize) void *pParameters, UINT32 ParameterByteSize) PURE;
};
...
#define IXAPO_LockForProcess(This,                                                                                     \
    ...
    ((This)->lpVtbl->LockForProcess(This,                                                                              \
                                    InputLockedParameterCount,                                                         \
                                    pInputLockedParameters,                                                            \
                                    OutputLockedParameterCount,                                                        \
                                    pOutputLockedParameters))
