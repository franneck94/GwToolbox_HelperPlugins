...
bool CmdTB(const wchar_t *, int argc, LPWSTR *argv)
{
    ...
    pluginname.erase(std::ranges::remove_if(pluginname, [](const wchar_t x) { return std::isspace(x); }).begin(),
                     pluginname.end());
    if (arg1.empty())
    {
        return false;
...
bool ToolboxUIPlugin::DrawTabButton(const bool show_icon, const bool show_text, const bool center_align_text)
{
    ImGui::PushStyleColor(ImGuiCol_Button,
                          plugin_visible ? ImGui::GetStyle().Colors[ImGuiCol_Button] : ImVec4(0, 0, 0, 0));
