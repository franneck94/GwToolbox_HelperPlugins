...
static void HeroUseSkill(const uint32_t hero_agent_id, const uint32_t target_agent_id, const uint32_t skill_id)
{
...
void HeroWindow::UseBipOnPlayer()
{
    ...
    {
        if (effect.skill_id == static_cast<uint32_t>(GW::Constants::SkillID::Blood_is_Power) &&
            effect.agent_id == player_data.id)
        {
            return;
        }
    }
        ...
        if (hero.owner_player_id == player_data.living->login_number)
        {
                ...
                dist < GW::Constants::Range::Spellcast && hero_living->hp > 0.80F)
            {
                if (HeroCastSkillIfAvailable(hero,
                                             hero_living,
                                             player_data.id,
                                             static_cast<uint32_t>(GW::Constants::SkillID::Blood_is_Power)))
                {
                    Log::Info("Casted BIP.");
                    return;
                }
...
bool HeroWindow::MesmerSpikeTarget(const GW::HeroPartyMember &hero)
{
    ...
    if (hero_living->primary == static_cast<uint8_t>(GW::Constants::Profession::Mesmer))
    {
        if (HeroCastSkillIfAvailable(hero,
                                     hero_living,
                                     target_agent_id,
                                     static_cast<uint32_t>(GW::Constants::SkillID::Energy_Surge)))
            return true;
    }
...
void HeroWindow::UseFallback()
{
    ...
    for (const auto effect : *effects)
    {
        if (effect.skill_id == static_cast<uint32_t>(GW::Constants::SkillID::Fall_Back))
            return;
    }
        ...
        if (hero.owner_player_id == player_data.living->login_number)
        {
            ...
            if (hero_living->secondary == static_cast<uint8_t>(GW::Constants::Profession::Paragon))
            {
                if (HeroCastSkillIfAvailable(hero,
                                             hero_living,
                                             target_agent_id,
                                             static_cast<uint32_t>(GW::Constants::SkillID::Fall_Back)))
                {
