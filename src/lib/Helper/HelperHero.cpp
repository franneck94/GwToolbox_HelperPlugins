                              ...
                              const Helper::Hero::TargetLogic target_logic)
{
    ...
    const auto player_id = GW::Agents::GetObservingId();
...
{
    const auto me_living = GW::Agents::GetControlledCharacter();
    if (!me_living)
        return;
...
void FlagHeros(const GW::GamePos &follow_pos)
{
    ...
    for (const auto &hero : party_info->heroes)
    {
        ...
        if (!me_living->GetIsMoving() && hero_distance > GW::Constants::Range::Spellcast)
            GW::PartyMgr::FlagHero(hero.agent_id, follow_pos);
...
void UnflagHeros()
{
    ...
    for (const auto &hero : party_info->heroes)
    {
        ...
        GW::PartyMgr::FlagHero(hero.agent_id, GW::GamePos(HUGE_VALF, HUGE_VALF, 0));
    }
}
} // namespace Hero
} // namespace Helper
