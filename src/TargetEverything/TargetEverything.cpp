...
void TargetEverything::Initialize(ImGuiContext *ctx, const ImGuiAllocFns fns, const HMODULE toolbox_dll)
{
    ...
    {
        // DEBUG toolbox?
        GetIsAgentTargettable_Func = reinterpret_cast<GetIsAgentTargettableFn>(GW::Scanner::Find(
            "\x55\x8B\xEC\x83\xEC\x0\xC7\x45\xF8\x0\x0\x0\x0\xC7\x45\xFC\x0\x0\x0\x0\x83\x7D\x08\x0\x75\x0\x32\xC0",
            "xxxxx?xxx????xxx????xxx?x?xx"));
    }
    if (!GetIsAgentTargettable_Func)
    {
    ...
    GW::HookBase::CreateHook(GetIsAgentTargettable_Func,
